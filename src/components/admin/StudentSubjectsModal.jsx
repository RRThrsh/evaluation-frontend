...
export default function StudentSubjectsModal({ student, subjects, config, onClose, onToast }) {
  ...
  const loadSubjects = async () => {
    try { const data = await api.get(`/api/admin/students/${student.id}/subjects`); setStudentSubjects(data.data ?? []); }
    catch (err) { onToast(err.message, "error"); }
  };

  const handleGrade = async (ssId, grade, status) => {
    try { await api.put(`/api/admin/students/grade/${ssId}`, { grade, status }); onToast("Grade saved"); await loadSubjects(); }
    catch (err) { onToast(err.message, "error"); }
  };
  ...
  const handleEnroll = async () => {
    ...
    setSaving(true);
    try { await api.post("/api/admin/students/enroll", { student_id: student.id, subject_id: enrollSubjectId }); onToast("Enrolled"); setEnrollSubjectId(""); await loadSubjects(); }
    catch (err) { onToast(err.message, "error"); }
  ...
  const loadCurriculum = async () => {
    ...
    try { const data = await api.get(`/api/admin/students/${student.id}/curriculum`); setCurriculum(data.data ?? []); setShowCurriculum(true); }
    catch (err) { onToast(err.message, "error"); }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {!managingSubjects && <>
              ...
              <button onClick={loadCurriculum} className="btn btn-ghost btn-sm">Logs</button>
            </>}
