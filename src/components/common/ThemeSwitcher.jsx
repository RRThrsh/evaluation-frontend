...
export default function ThemeSwitcher() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((p) => !p)}
        className="flex items-center justify-center w-9 h-9 rounded-xl border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition"
        title="Change theme"
      >
        <Palette size={16} />
      </button>
        ...
        <div className="theme-dropdown py-1">
          {themes.map((t) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition hover:bg-slate-50 ${
                theme === t ? "font-semibold text-slate-900" : "text-slate-600"
              }`}
            >
