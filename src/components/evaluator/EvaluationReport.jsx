          ...
          {next_semester_subjects.map((ns, i) => (
            <div key={i} className={`p-3 flex items-center justify-between transition hover:brightness-90 ${ns.is_retake ? "bg-amber-50" : ""}`}>
              <div>
                ...
                {ns.is_gap_filler && <span className="text-purple-600 text-xs ml-1 font-bold">[GAP]</span>}
                {ns.is_retake && !ns.is_gap_filler && <span className="text-amber-600 text-xs ml-2 font-bold">[RETAKE]</span>}
              </div>
